"""
Analysis tools for the Multi-Agent RCA System.

Provides tools for parsing and analyzing error traces and logs.
"""
...
def _extract_stack_frames(stack_details: str) -> List[Dict[str, Any]]:
    """
    Parse the stack_details JSON string and extract relevant frames.
    
    Args:
        stack_details: JSON string containing stack frame information
...
def _filter_internal_frames(frames: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Filter out external/library frames, keeping only application code.
    
    Args:
        frames: List of stack frame dictionaries
        
    Returns:
        Filtered list containing only internal (application) frames
    """
    internal_frames = []
    for frame in frames:
...
def parse_error_trace(trace_path: str) -> str:
    ...
    try:
        ...
        if not path.is_absolute():
            # Try the default error trace path
            if not path.exists():
        ...
        if isinstance(trace_data, list):
            ...
            trace_data = trace_data[0]  # Take first error
        ...
        # Add the primary error location
        if internal_frames:
