...
def _validate_path(path: str, allow_absolute: bool = True) -> Path:
    ...
    try:
        ...
        if ".." in path:
            ...
            pass  # resolve() handles this, but we're being explicit
...
def read_file(file_path: str, start_line: Optional[int] = None, end_line: Optional[int] = None) -> str:
    ...
    """
    try:
        # Handle Docker path mapping first
        # Docker paths like /usr/srv/app/services/user.py -> fastapi-project/app/services/user.py
        original_path = file_path
        if file_path.startswith("/usr/srv/app/"):
            # Docker path - map to local codebase
        ...
        if path.is_absolute():
            ...
            candidates.extend([
                CODEBASE_PATH / "app" / file_path,  # Most likely for services/user.py
                CODEBASE_PATH / file_path,          # Direct under codebase
                Path(file_path),                     # As-is
            ])
            
            # If path starts with services/, models/, etc., try under app/
        ...
        # Add line numbers for easier reference (optionally sliced)
        lines = content.split("\n")
